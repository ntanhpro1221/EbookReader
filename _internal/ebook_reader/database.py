...
SEGMENT_CEILING_METRIC_KEY = "generation_ceiling_hit"
...
CREATE TABLE IF NOT EXISTS machine_take_substitutions (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    segment_stable_id TEXT NOT NULL,
...
class ProjectDB:
    ...
    def chapter_is_publishable(self, chapter_id: int) -> bool:
        ...
        with self.connect() as conn:
            ...
            if not rows:
                return False
            # Cả hai nguồn phán xử, vì cổng này chỉ hỏi "bản thu đã được ai đó cho đi
    ...
    def accept_segment_audio_as_machine(
        self,
        *,
        segment_stable_id: str,
        wav_sha256: str,
    ...
    def _require_candidate_beats_a_cut_off_incumbent(
        self,
        candidate: sqlite3.Row,
        segment: sqlite3.Row,
    ) -> str:
    ...
    def chapter_segments_have_current_audio_qa(
        ...
        with self.connect() as conn:
            ...
            accepted = self._ruled_takes_conn(conn)
            for row in rows:
