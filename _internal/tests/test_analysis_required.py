...
V30_SEQ23_TEXT = (
    "Bên kia cánh cửa gỗ lung lay sắp rớt là một cái bếp lò nhìn không ra màu sắc ban "
    "đầu, bên trên treo một chiếc bình sành, củi bên dưới không biết đã tắt được bao lâu, "
    "chỉ còn một chút hơi nóng phả ra."
)
V30_SEQ24_TEXT = (
    "Mọi thứ đều thật xa lạ, Hạ Phong căn bản không thể đoán được mình đang ở đâu. Mà "
...
def v29_seq37_39_rows():
    return [
        {
            "id": 38,
            "stable_id": "c00001_s0000037_55bf068518ab",
            "chapter_id": 1,
            "seq": 37,
            "paragraph_index": 32,
            "text": V29_SEQ37_TEXT,
            "text_sha256": sha256_text(V29_SEQ37_TEXT),
            "kind_hint": "dialogue",
            "status": "pending",
            "speaker": None,
        },
        {
            "id": 39,
            "stable_id": "c00001_s0000038_6d1eebb237de",
            "chapter_id": 1,
            "seq": 38,
            "paragraph_index": 33,
            "text": V29_SEQ38_TEXT,
        ...
        {
            "id": 40,
            "stable_id": "c00001_s0000039_55abbb7a19a4",
            "chapter_id": 1,
            "seq": 39,
            "paragraph_index": 33,
            "text": V29_SEQ39_TEXT,
            "text_sha256": sha256_text(V29_SEQ39_TEXT),
            "kind_hint": "thought",
            "status": "pending",
            "speaker": None,
        },
...
def v30_seq23_25_rows():
    return [
        {
            "id": 24,
            "stable_id": "c00001_s0000023_e1c67912cf39",
            "chapter_id": 1,
            "seq": 23,
            "paragraph_index": 20,
            "text": V30_SEQ23_TEXT,
            "text_sha256": sha256_text(V30_SEQ23_TEXT),
            "kind_hint": "narration",
            "status": "analyzed",
            "speaker": "NARRATOR",
        },
        {
            "id": 25,
            "stable_id": "c00001_s0000024_3ab59bb4bd38",
            "chapter_id": 1,
            "seq": 24,
            "paragraph_index": 21,
            "text": V30_SEQ24_TEXT,
...
def test_generator_schema_forbids_free_form_analysis_metadata() -> None:
    ...
    assert "Việc chỉ ưa một phương án delivery khác không đủ" in (
        DIRECTOR_CRITIC_SYSTEM_PROMPT
    )
    assert "hỗn loạn” khi đứng một mình không bắt buộc" in (
        DIRECTOR_CRITIC_SYSTEM_PROMPT
    )
...
def test_v29_seq38_generator_cannot_relabel_narration_lead_in_as_thought() -> None:
    ...
    assert [
        issue.canonical_payload()
        for issue in _source_kind_feedback_issues(
            [row],
            payload,
            original_context=original_context,
        )
...
@pytest.mark.parametrize(
    ("target_overrides", "next_overrides"),
    [
        ({}, {"kind_hint": "narration"}),
        ({}, {"paragraph_index": 34}),
        ({}, {"seq": 40}),
...
def test_context_kind_lock_requires_exact_immutable_narration_thought_transition(
    target_overrides: dict[str, Any],
    next_overrides: dict[str, Any],
) -> None:
    source_rows = v29_seq37_39_rows()
    source_rows[1].update(target_overrides)
    source_rows[2].update(next_overrides)
    row = source_rows[1]
    stable_id = str(row["stable_id"])
    item = analysis_item(stable_id)
    item.update({"kind": "thought", "speaker": "NARRATOR"})
    ...
    assert validated[stable_id]["kind"] == "thought"
    assert _source_kind_feedback_issues(
        [row],
        payload,
        original_context=original_context,
    ) == ()
...
@pytest.mark.parametrize(
    ...
    [
        V29_SEQ38_TEXT,
        "Thấy vẻ mặt thất thần của Hạ Phong, cậu bé cũng không lấy làm lạ:",
        "Cậu bàng hoàng đứng lặng giữa căn phòng.",
    ],
)
def test_disoriented_cues_do_not_reject_or_advise_neutral_delivery(text: str) -> None:
    stable_id = "disoriented-neutral"
    ...
    assert _direct_cue_feedback_issues(group, validated, issues) == ()


def test_disoriented_cue_remains_non_happy_evidence() -> None:
    stable_id = "disoriented-happy"
    group = [
        {
            "stable_id": stable_id,
            "text": "Cậu thất thần nhìn căn phòng hỗn loạn.",
            "kind_hint": "narration",
        }
    ]
    candidate = analysis_item(stable_id)
    candidate["emotion"] = "happy"

    issues, batch_collapsed = _semantic_delivery_issues(
        group,
        {stable_id: candidate},
    )

    assert batch_collapsed is False
    assert 'disoriented="thất thần"' in issues[stable_id]


def test_explicit_fear_remains_direct_when_disorientation_is_also_present() -> None:
    stable_id = "disoriented-with-fear"
    group = [
        {
            "stable_id": stable_id,
            "text": "Tâm trí anh hỗn loạn vì hoảng sợ.",
            "kind_hint": "narration",
        }
    ]
    validated = {stable_id: analysis_item(stable_id)}
    ...
    issues, batch_collapsed = _semantic_delivery_issues(group, validated)
    feedback = _direct_cue_feedback_issues(group, validated, issues)

    assert batch_collapsed is False
    assert 'afraid="hoảng sợ"' in issues[stable_id]
    assert feedback == (
        AnalysisFeedbackIssue(
            stable_id=stable_id,
            code="SEMANTIC_DELIVERY_MISMATCH",
...
def test_v30_seq24_masks_next_paragraph_thought_from_critic_without_host_lock() -> None:
    ...
    assert returned_hash == candidate_hash
    critic_row = candidate_rows[0]
    assert original_context[stable_id]["next_text"] == V30_SEQ25_TEXT
    assert critic_row["context_policy"] == (
        ANALYSIS_CONTEXT_POLICY_NARRATION_BEFORE_NEXT_PARAGRAPH_THOUGHT
    )
    assert critic_row["previous_text"] == V30_SEQ23_TEXT
    assert critic_row["next_text"] == ""
    assert critic_row["host_locked_fields"] == {}
    request = session.request["json"]
    assert V30_SEQ23_TEXT in request["prompt"]
    assert V30_SEQ24_TEXT in request["prompt"]
    assert V30_SEQ25_TEXT not in request["prompt"]
    assert "narration_precedes_next_paragraph_thought" in request["system"]
    assert "Policy này" in request["system"]
    assert "không khóa field nào; chỉ đánh giá" in request["system"]


def test_v30_seq24_generator_keeps_next_paragraph_thought_context() -> None:
    source_rows = v30_seq23_25_rows()
    row = source_rows[1]
    response = analysis_item("S001")
    session = FakeSession({"segments": [response]})
    ...
    assert payload["segments"][0]["kind"] == "narration"
    prompt = session.request["json"]["prompt"]
    assert V30_SEQ23_TEXT in prompt
    assert V30_SEQ24_TEXT in prompt
    assert V30_SEQ25_TEXT in prompt


def test_v30_seq24_next_thought_mask_is_invariant_across_batch_shapes() -> None:
    source_rows = [
        {
            "id": 22,
            "stable_id": "v30-seq21",
        ...
        },
        {
            ...
            "seq": 22,
            "paragraph_index": 19,
            "text": "Cậu chậm rãi nhìn quanh căn phòng.",
            "kind_hint": "narration",
        },
        *v30_seq23_25_rows(),
    ]
    ...
    original_context = _original_neighbor_context(source_rows)
    groups = (source_rows, source_rows[-2:], [target])

    assert [len(group) for group in groups] == [5, 2, 1]
    for group in groups:
        validated = {}
        for row in group:
            stable_id = str(row["stable_id"])
        ...
        critic_row = next(
            item for item in critic_rows if item["text"] == V30_SEQ24_TEXT
        )
...
def test_narration_lead_in_mask_rejects_unsupported_next_source(
    next_kind_hint: str,
...
def test_next_paragraph_thought_mask_requires_exact_source_transition(
    target_overrides: dict[str, Any],
    next_overrides: dict[str, Any],
) -> None:
    source_rows = v30_seq23_25_rows()
...
def test_v30_seq24_mask_keeps_critic_kind_and_affect_deltas_live(monkeypatch) -> None:
    ...
    assert candidate_rows[0]["host_locked_fields"] == {}
    assert issues == {
        stable_id: "DIRECTOR_FIELD_MISMATCH fields=kind,emotion,intensity,pace",
    }
    item = evidence["segments"][0]
...
def test_v29_seq38_kind_only_critic_dissent_uses_context_bound_override() -> None:
    ...
    assert candidate_rows[0]["host_locked_fields"] == {"kind": "narration"}
    assert issues == {}
    item = evidence["segments"][0]
    assert item["critic"]["accept"] is False
    ...
    assert item["host_source_kind_override"] == {
        ...
        "unresolved_field_deltas": [],
        "related_stable_id": source_rows[2]["stable_id"],
        "related_text_sha256": sha256_text(V29_SEQ39_TEXT),
    }
...
def test_v29_seq38_context_override_covers_only_kind_delta(monkeypatch) -> None:
    ...
    assert issues == {
        stable_id: "DIRECTOR_FIELD_MISMATCH fields=emotion,intensity,pace",
    }
    item = evidence["segments"][0]
    assert item["field_deltas"] == [
        "kind:narration->thought",
...
def test_context_kind_and_sleep_emotion_overrides_compose_with_context_rule_priority(monkeypatch) -> None:
    ...
    row = {
        "id": 11,
        "stable_id": "context-and-sleep-lock",
        "chapter_id": 1,
        "seq": 10,
        "paragraph_index": 9,
        "text": V28_SEQ10_TEXT,
    ...
    next_row = {
        "id": 12,
        "stable_id": "context-and-sleep-thought",
        "chapter_id": 1,
        "seq": 11,
        "paragraph_index": 9,
        "text": next_text,
        "text_sha256": sha256_text(next_text),
        "kind_hint": "thought",
    }
    original_context = _original_neighbor_context([row, next_row])
    validated = {
        row["stable_id"]: {
            **analysis_item(row["stable_id"]),
            "emotion": "afraid",
    ...
    assert item["host_source_kind_override"]["related_stable_id"] == next_row[
        "stable_id"
    ]
...
def test_persistent_director_rejection_splits_then_publishes_the_singleton(
    monkeypatch,
) -> None:
    """Was "...then fails singleton" until the owner changed the policy on 2026-09-05.

    The escalation asserted below is unchanged and is the point of this test: retry, split,
    retry, split, down to singletons. What changed is only what happens when a singleton
    still cannot be agreed - it used to end the book, and now it publishes the model's own
    reading with a warning for the owner's ear. `pace` is audible, so this takes the new
    last-resort path rather than the emotion/intensity one.
    """
    db = FakeDB()
    db.rows = [
        {
    ...
    # 4 -> 2+2, and each half -> 1+1. The old expectation stopped at [4,4,2,2,1,1] because
    # the first singleton ended the book; the run now works through all four segments,
    # which is the whole point of the change.
    assert generator_sizes == [4, 4, 2, 2, 1, 1, 1, 1, 2, 2, 1, 1, 1, 1]
    assert critic_sizes == [4, 2, 1, 1, 2, 1, 1]
    assert not any(event[1] == "REQUIRED_ANALYSIS_BATCH_FAILED" for event in db.events)
    assert any(
        event[1] == "ANALYSIS_AUDIBLE_DISAGREEMENT_ACCEPTED" for event in db.events
    )
    assert db.updated, "the book continues, so the segments must actually be written"
...
# after the director critic has already accepted. A critic rejection has no acceptance
# record, so a batch that cannot be split had nothing left and ended the book, taking ten
# chapters of finished analysis with it.
#
# These drive the real analyze_all. An earlier attempt passed its unit tests and still
# failed here, on a second guard - "Phản biện đạo diễn bắt buộc thiếu evidence" - that no
# unit test could have seen.


def _one_dialogue_segment_db(tmp_path):
    """A batch of one, so len(group) > 1 is false and no split is possible."""
    db = ProjectDB(tmp_path / "project.sqlite3")
    db.initialize_book(
        title="Book",
        project_root=tmp_path,
        ...
        [
            {
                ...
                "text": text,
                "text_sha256": sha256_text(text),
                # dialogue on purpose: _heuristic returns UNKNOWN for it, so the committed
                # speaker says which answer was actually used.
                "kind_hint": "dialogue",
            }
        ],
    )
...
def _critic_that_always_disputes_the_delivery(db, monkeypatch, model_speaker):
    analyzer = OllamaBookAnalyzer(build_settings(), db, lambda _message: None)
    monkeypatch.setattr(analyzer, "ensure_available", lambda: True)
    monkeypatch.setattr(
        analyzer,
    ...
    def dispute_the_delivery(_self, group, validated, **kwargs):
        # `pace` and not `speaker`: correcting the speaker to a name the text never uses
        # makes the critic's own answer fail provenance, which the system records as
        # DIRECTOR_INVALID_RESPONSE - a malfunctioning critic, not a disagreement, and a
        # case that deliberately still fails. `pace` is audible and provenance-free, so it
        # produces the DIRECTOR_FIELD_MISMATCH this path is actually for, the same reason
        # string the real run died on.
        return director_critic_payload(
            group,
            validated,
            corrections={0: {"pace": "fast"}},
            candidate_rows=kwargs.get("candidate_rows"),
            candidate_hash=kwargs.get("candidate_hash"),
        )
...
def test_an_unsplittable_audible_disagreement_does_not_end_the_book(
    tmp_path, monkeypatch
) -> None:
    db = _one_dialogue_segment_db(tmp_path)
    analyzer = _critic_that_always_disputes_the_delivery(db, monkeypatch, "LUCIEN")

    analyzer.analyze_all(lambda: False)  # must not raise

    assert "ANALYSIS_AUDIBLE_DISAGREEMENT_ACCEPTED" in _codes(db)
    assert "REQUIRED_ANALYSIS_BATCH_FAILED" not in _codes(db)


def test_it_publishes_the_model_answer_rather_than_a_heuristic_guess(
    tmp_path, monkeypatch
) -> None:
    """Surviving is not enough. _heuristic hands dialogue an UNKNOWN speaker, which is a
    worse reading than the disputed one, delivered quietly."""
    db = _one_dialogue_segment_db(tmp_path)
    analyzer = _critic_that_always_disputes_the_delivery(db, monkeypatch, "LUCIEN")
    ...
    assert str(segment["speaker"]) == "LUCIEN", "the model's own schema-valid answer"
    assert str(segment["speaker"]) != "UNKNOWN", "which is what _heuristic would give"


def test_the_committed_batch_is_never_recorded_as_critic_accepted(
    tmp_path, monkeypatch
) -> None:
    """The committed event is what a later audit reads. Calling this 'accepted' would hide
    the one fact that matters about these segments."""
    db = _one_dialogue_segment_db(tmp_path)
    analyzer = _critic_that_always_disputes_the_delivery(db, monkeypatch, "LUCIEN")
    ...
    codes = _codes(db)
    assert "ANALYSIS_DIRECTOR_CRITIC_PROTESTED" in codes
    assert "ANALYSIS_DIRECTOR_CRITIC_ACCEPTED" not in codes


def test_the_ledger_still_says_the_critic_rejected_it(tmp_path, monkeypatch) -> None:
    """Nothing false is written: the candidate is not bound to the commit and its state is
    left exactly as the critic left it."""
    db = _one_dialogue_segment_db(tmp_path)
    analyzer = _critic_that_always_disputes_the_delivery(db, monkeypatch, "LUCIEN")
    ...
    with db.connect() as conn:
        states = {
            str(row["state"])
            for row in conn.execute("SELECT state FROM analysis_candidates")
        }
    assert "accepted" not in states
    assert states <= {"critic_rejected", "terminal"}, states


def test_the_warning_names_the_segment_and_field_a_listener_must_check(
    tmp_path, monkeypatch
) -> None:
    """A warning with no segment id cannot send anybody to the right forty seconds."""
    db = _one_dialogue_segment_db(tmp_path)
    analyzer = _critic_that_always_disputes_the_delivery(db, monkeypatch, "LUCIEN")
    ...
    row = next(
        r for r in db.list_events()
        if str(r["code"]) == "ANALYSIS_AUDIBLE_DISAGREEMENT_ACCEPTED"
    )
    details = json.loads(str(row["details_json"]) or "{}")
    assert details["disputed_segments"] == ["c1s1"]
    assert "pace" in details["fields"]
    assert details["needs_listener"] is True


def test_a_malfunctioning_critic_still_ends_the_batch(tmp_path, monkeypatch) -> None:
    """The boundary of the owner's decision, and it is narrow on purpose.

    They ruled on a critic that read the delivery and disagreed. A critic that returns an
    unusable answer has not disagreed with anything - it has malfunctioned - and publishing
    over a malfunction is a different decision nobody has made. Correcting the speaker to a
    name the text never uses makes the critic's own answer fail provenance, which is
    recorded as DIRECTOR_INVALID_RESPONSE rather than DIRECTOR_FIELD_MISMATCH.
    """
    db = _one_dialogue_segment_db(tmp_path)
    analyzer = OllamaBookAnalyzer(build_settings(), db, lambda _message: None)
    monkeypatch.setattr(analyzer, "ensure_available", lambda: True)
    monkeypatch.setattr(
        ...
        lambda group, **_kwargs: {
            "segments": [
                {**analysis_item(str(row["stable_id"])), "kind": "dialogue", "speaker": "LUCIEN"}
                for row in group
            ]
        },
    )

    def malfunction(_self, group, validated, **kwargs):
        return director_critic_payload(
            group,
            validated,
            corrections={0: {"speaker": "AI_KHAC"}},
            candidate_rows=kwargs.get("candidate_rows"),
            candidate_hash=kwargs.get("candidate_hash"),
        )
    ...
    assert "ANALYSIS_AUDIBLE_DISAGREEMENT_ACCEPTED" not in _codes(db)
